...
export const Goals = () => {
  ...
  return (
      <PageTemplate>
          <div className="flex flex-col bg-gray-100 dark:bg-gray-900 w-full h-full">
              <div
                  className="bg-blue-100 dark:bg-gray-800 p-2 h-[60px] text-center flex items-center justify-between w-full">
                  <h2 className="text-center text-blue-600 dark:text-blue-400 text-3xl font-bold">Twoje Cele</h2>
                  <button onClick={openAddGoalForm}
                          className="bg-blue-500 dark:bg-blue-600 text-white px-4 py-2 rounded">Dodaj Cel
                  </button>
              </div>
              <div
                  className="h-auto p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 bg-gray-100 dark:bg-gray-900 overflow-y-auto scrollbar-none md:scrollbar scrollbar-w-1.5 scrollbar-thumb-rounded-full scrollbar-thumb-blue-500 dark:scrollbar-thumb-gray-500">
                  {goals.map((goal) => (
                      <GoalCard
                          key={goal.id}
                          id={goal.id}
              ...
              {showAchievementMessage && (
                  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                      <div className="bg-white dark:bg-gray-800 p-6 rounded shadow-lg text-center">
                          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Cel Osiągnięty!</h2>
                          <p className="mb-4 text-black dark:text-gray-200">Gratulacje! Udało Ci się osiągnąć
                              cel: <strong>{currentGoalName}</strong></p>
                      </div>
                  </div>
              )}
          </div>
          <PopUpBg onClose={handleOnClose} visible={showPopup}>
              ...
              {currentForm === 'editGoal' &&
                  <GoalForm onClose={handleOnClose} onGoalCreated={handleGoalCreated} goalId={currentGoalId}
                            initialData={initialData}/>}
              {currentForm === 'donate' && (
                  <div className="p-4 bg-white dark:bg-gray-800 rounded">
                      <h1 className="text-center text-2xl font-bold mb-6 text-black dark:text-gray-100">Wpłać na
                          Cel: {currentGoalName}</h1>
                      <div className="mb-4">
